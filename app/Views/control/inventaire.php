                ...
                <div class="mt-1 mb-3 p-3 button-container bg-white border shadow-sm lh-sm">
                    ...
                    <div id="header" class="tabulator-print-header">
                        <div style="display: box;margin-top:5%">
                            ...
                            <img src="<?= base_url() ?>/assets/img/prof1.jpg" width="110px" alt="Logo Institut Pasteur">
                        ...
                        <div style="margin-left:200px;margin-bottom:50px;">
                            <h6 style="letter-spacing: 35px;">INVENTAIRE DE STOCK</h6>
                        </div>
                    </div>
                    <div>
                        <div class="email-msg">
        ...
        <?php if (isset($inventory)) {
            ...
            foreach ($inventory  as $e) {
                ...
                $tabledata[] = [
                    ...
                    "qtite_prod" => esc($e->Stock_Disponible),
                    "stock_theor" => esc($e->Stock_Theorique),
                    "compt_phys" => esc($e->Comptage_Physique),
                    "ecart" => esc($e->Ecart),
                ];
                $index++;
            }
            ...
            columns: [{
                ...
                },
                {
                    ...
                    cellEdited: function(cell) {
                        var cellValue = cell.getValue(); //Nouvelle valeur de la cellule
                        var rowData = cell.getRow().getData(); //les données de la ligne entière
                        var columnField = cell.getField(); //Le  champ de la colonne

                        $.ajax({
                            url: '/mod_comptage_phys',
                            method: 'POST',
                            data: {
                                'id': rowData['code'],
                                'compt_phys': rowData['compt_phys'],
                                'qtite_prod': rowData['qtite_prod'],
                            },
                            dataType: 'json',
                        }).done(function(data) {
                            swal("Ok", "Modification réussie", {
                                icon: "success",
                            }).then(function() {
                                setTimeout(() => {
                                    location.reload();
                            ...
                            swal("Ok", "Une erreur est survenue", {
                                icon: "error",
                            });
                        });
                    },
