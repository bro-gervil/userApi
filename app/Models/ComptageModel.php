...
class ComptageModel extends Model
{
    ...
    protected $allowedFields    = ['id','id_prod','stock_theor','ecart','compt','created_at'];
    ...
    public function toselectCompt($idp){
        $db= \Config\Database::connect();
        
        $query=$db->query('SELECT id, compt, stock_theor FROM comptage WHERE id_prod ='.$idp);
        $result=$query->getResultArray();
        $data = array();
        foreach($result as $r){
            $data = [
                "id" => $r['id'],
                "stock_theor" => $r['stock_theor'],
                "compt" => $r['compt'],
            ];
        }
        return $data; 
    ...
    public function update_comptage($data2,$data1,$ecart){
        $query = $this->db->table('comptage')->where("id_prod",$data2)->update(['compt'=>$data1,'ecart'=>$ecart,]);
        return $query;
    }
